"""
V-SPREAD-CONFIRM — μ* spread as key variable for CLAIM-60, N=100 (GAE 0.7.8)
==============================================================================
Persona A geometry. device_trust enriched (σ 0.200→0.100), spread=0.30.
Primary factors (threat_intel, pattern_history) unchanged, spread=0.62.
W-share post: device_trust ≈ 11.9% of total kernel weight.

Hypothesis: low μ* spread on enriched factor produces low/no Day-1 delta
even when W_boot is elevated (400.0).

Sanity: if Day-1 > +1.5pp: STOP immediately.

Run:
    PYTHONUTF8=1 python experiments/v_spread_confirm/run.py
"""
...
W_BOOT_DT = SIGMA_BEFORE["device_trust"]**2 / SIGMA_AFTER["device_trust"]**4   # 400.0
...
assert abs(_dt_spread - 0.30) < 0.01, f"device_trust spread={_dt_spread:.3f}, expected 0.30"
assert abs(_ti_spread - 0.62) < 0.01, f"threat_intel spread={_ti_spread:.3f}, expected 0.62"
...
def main():
    print("="*65)
    print("V-SPREAD-CONFIRM (GAE 0.7.8, N=100)")
    print("="*65)
    print(f"Enriched: device_trust  spread={_dt_spread:.2f}  "
          f"W_boot={W_BOOT_DT:.1f}  W_share={W_SHARE_PCT:.1f}%")
    print(f"Primary:  threat_intel  spread={_ti_spread:.2f}  (unchanged)")
    print()
    ...
    # ── Sanity check ───────────────────────────────────────────────────────────
    if d1_delta > 1.5:
        print(f"STOP — Day-1 delta={d1_delta:+.2f}pp > +1.5pp anomaly threshold.")
        print("Geometry or sigma mismatch. Do not continue.")
        return

    if obs_d > 0.20:
        print(f"FLAG: d={obs_d:.4f} > 0.20 (suspicious for device_trust spread=0.30)")

    spread_confirmed = bool(d1_delta <= 1.5 and obs_d <= 0.20)

    results = {
        "experiment":              "V-SPREAD-CONFIRM",
        "gae_version":             "0.7.8",
        "enriched_factor":         "device_trust",
        "enriched_factor_spread":  round(_dt_spread, 3),
        "primary_factor_spread":   round(_ti_spread, 3),
        "w_share_post_pct":        round(W_SHARE_PCT, 2),
        ...
        "spread_hypothesis_confirmed": spread_confirmed,
        "vcga_v3_asset_crit_sigma":  0.060,
        "vcga_v3_time_anomaly_sigma": 0.070,
        "b0p2_device_trust_spread":  0.0,
        "runtime_s":               round(elapsed, 1),
        "note": "Raw numbers for roadmap session only",
    }
    ...
    label = "CONFIRMED" if spread_confirmed else "NOT CONFIRMED"
    print()
    print("="*65)
    print("V-SPREAD-CONFIRM:")
    print(f"  Retrieved — V-CGA-v3: asset_crit=0.060, time_anomaly=0.070")
    print(f"  Retrieved — B0-P2 device_trust spread: 0.0 (flattened)")
    print(f"  Enriched: device_trust (spread={_dt_spread:.2f}, secondary)")
    print(f"  W-share post: {W_SHARE_PCT:.1f}%")
    print(f"  Day-1: {d1_delta:+.2f}pp  d={obs_d:.4f}  "
          f"p={float(p):.4f}  CI=[{ci[0]:.2f},{ci[1]:.2f}]")
    print(f"  n_half C0={nh_c0.mean():.1f}  T2={nh_t2.mean():.1f}")
    print(f"  Final delta: {(fa_t2-fa_c0)*100:+.2f}pp")
    print(f"  Spread hypothesis: {label}")
