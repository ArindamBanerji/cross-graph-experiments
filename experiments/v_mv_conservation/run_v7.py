"""
V-MV-CONSERVATION v7 — 32-cell design, onset ∈ {50, 100} only (GAE 0.7.14).

onset=200 removed: 115-decision gap between plateau (t≈55) and onset (t=200)
is structurally incompatible with q_endpoint∈{0.65,0.70} degradation magnitude.
v6 threshold sweep confirmed no single threshold resolves both precision and
recall for onset=200. onset=50 and onset=100 consistently pass at threshold=0.05
(P≈0.73, R≈0.98 each).

Claim scope: "detects degradation within 100 decisions of onset."

Save: experiments/v_mv_conservation/results/results_v7.json
"""
...
ONSET_TIMES   = [50, 100]        # onset=200 removed
...
# ---------------------------------------------------------------------------
# SOC geometry (unchanged from v2-v6)
# ---------------------------------------------------------------------------
...
def run_seed(onset_time: int, deg_profile: str, q_endpoint: float,
    ...
    if varq_mon.fired:
        outcome = "TP" if (tp_low <= varq_mon.fire_t <= tp_high) else "FP"
    else:
        outcome = "FN"

    lag = (varq_mon.fire_t - onset_time) if varq_mon.fired else None
...
def main():
    ...
    n_cells = len(ONSET_TIMES) * len(DEG_PROFILES) * len(Q_ENDPOINTS) * len(SIGMA_V_PAIRS)
    print(f"V-MV-CONSERVATION v7 (GAE {gae.__version__})")
    print(f"  {n_cells} cells x {N_SEEDS} seeds = {n_cells*N_SEEDS} runs "
          f"(onset=200 removed, threshold=0.05)", flush=True)
    print()
    ...
    for cell_idx, (onset_time, deg_profile, q_endpoint, (sigma_eff, V)) in enumerate(cells):
        ...
        print(f"  [{cell_idx+1:02d}/32] onset={onset_time:3d} {deg_profile:6s} "
    ...
    # ---------------------------------------------------------------------------
    out = {
        "experiment":   "V-MV-CONSERVATION-v7",
        "gae_version":  "0.7.14",
        "design_note":  "onset=200 removed — 115-decision gap structurally incompatible",
        "n_cells":      32,
        "n_seeds":      N_SEEDS,
    ...
    # ---------------------------------------------------------------------------
    print()
    print(f"V-MV-CONSERVATION v7 (32 cells, onset∈{{50,100}}, threshold=0.05):")
    print(f"  VarQMonitor:")
    ...
    print(f"  Claim scope: detects degradation within 100 decisions of onset.")
