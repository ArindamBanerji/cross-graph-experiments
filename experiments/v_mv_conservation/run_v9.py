"""
V-MV-CONSERVATION v9 — VarQMonitor baseline_window=30, all 48 cells (GAE 0.7.16).

Identical design to v8 (48 cells, all onset times). Single change: GAE 0.7.16
reduces VarQMonitor.baseline_window from 50 to 30, counting override calls
passed to update(). With ALPHA=0.25, onset=100 gets ~25 pre-onset healthy
overrides in the baseline window (vs ~12 for onset=50).

v8 failure: baseline_window=50 overrides → all 3 onset times contaminated or
failing (onset=50/100 P=R=0; onset=200 P=0.603/R=0.691).
v9 fix: baseline_window=30 overrides → fewer contaminated baseline calls.

VarQMonitor API (GAE 0.7.16): called once per override, q_t = quality (0.0/1.0).
update(q_t) returns True on persistence-3 crossing. Track fire_t externally.

Save: experiments/v_mv_conservation/results/results_v9.json
"""
...
# ---------------------------------------------------------------------------
# Parameters (unchanged from v2-v8)
# ---------------------------------------------------------------------------
N_SEEDS      = 20
N_DECISIONS  = 350
ALPHA        = 0.25
...
# ---------------------------------------------------------------------------
# SOC geometry (unchanged from v2-v8)
# ---------------------------------------------------------------------------
...
def run_seed(onset_time: int, deg_profile: str, q_endpoint: float,
    ...
    monitor = VarQMonitor()   # threshold=0.05, persistence=3, window=30, baseline_window=30
...
def main():
    import gae
    assert gae.__version__ == "0.7.16", f"Need GAE 0.7.16, got {gae.__version__}"
    print(f"V-MV-CONSERVATION v9 (GAE {gae.__version__})")
    print(f"  48 cells x {N_SEEDS} seeds = {48*N_SEEDS} runs "
          f"(VarQMonitor persistence=3, baseline_window=30)", flush=True)
    print()
    ...
    # ---------------------------------------------------------------------------
    out = {
        "experiment":                  "V-MV-CONSERVATION-v9",
        "gae_version":                 "0.7.16",
        "detector":                    "VarQMonitor",
        "persistence":                 3,
        "threshold":                   0.05,
        "window":                      30,
        "baseline_window_decisions":   30,
        "n_cells":                     48,
        "n_seeds":                     N_SEEDS,
        ...
        "series_summary": {
            "v6_no_persistence_onset200_precision": 0.301,
            "v8_persistence_onset50_precision":     0.000,
            "v9_precision":                         m_all["precision"],
            "v9_recall":                            m_all["recall"],
        },
        "cells": cell_records,
    }

    out_path = (REPO_ROOT / "experiments" / "v_mv_conservation"
                / "results" / "results_v9.json")
    ...
    # ---------------------------------------------------------------------------
    print()
    print(f"V-MV-CONSERVATION v9 (48 cells, all onset times, GAE 0.7.16):")
    print(f"  VarQMonitor (persistence=3, threshold=0.05, baseline=30d):")
    ...
    print(f"  Series: v6(no persist) → v8(persist,wrong baseline) → v9(persist,30d baseline)")
