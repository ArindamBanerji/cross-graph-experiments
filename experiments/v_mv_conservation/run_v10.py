"""
V-MV-CONSERVATION v10 — VarQMonitor baseline_window=10, all 48 cells (GAE 0.7.17).

Identical design to v8/v9. Single change: baseline_window=10 override observations.
At ALPHA=0.25, onset=50 yields ~12 pre-onset overrides → baseline fills at 10 with
2 clean overrides to spare. Onset=100: ~25 clean overrides → clean baseline.
Onset=200: ~50 clean overrides → clean baseline.

v9 failure: baseline_window=30 overrides → onset=50 had only ~12 healthy overrides,
contaminating 18/30 baseline slots → var_norm=0.031 < threshold=0.05 → never fires.
v8 confirmed persistence filter: onset=200 P: 0.301→0.603 (+30.2pp).
v7 confirmed onset=50/100 clean-baseline detection: P=0.734/0.722, R=0.983.

Save: experiments/v_mv_conservation/results/results_v10.json
"""
...
# ---------------------------------------------------------------------------
# Parameters (unchanged from v2-v9)
# ---------------------------------------------------------------------------
N_SEEDS      = 20
N_DECISIONS  = 350
ALPHA        = 0.25
...
# ---------------------------------------------------------------------------
# SOC geometry (unchanged from v2-v9)
# ---------------------------------------------------------------------------
...
def run_seed(onset_time: int, deg_profile: str, q_endpoint: float,
    ...
    monitor = VarQMonitor()   # threshold=0.05, persistence=3, window=30, baseline_window=10
...
def main():
    ...
    print(f"V-MV-CONSERVATION v10 (GAE {gae.__version__})")
    print(f"  48 cells x {N_SEEDS} seeds = {48*N_SEEDS} runs "
          f"(VarQMonitor persistence=3, baseline_window=10 overrides)", flush=True)
    print()
    ...
    # ---------------------------------------------------------------------------
    out = {
        "experiment":                   "V-MV-CONSERVATION-v10",
        "gae_version":                  "0.7.17",
        "detector":                     "VarQMonitor",
        "persistence":                  3,
        "threshold":                    0.05,
        "window":                       30,
        "baseline_window_overrides":    10,
        "n_cells":                      48,
        "n_seeds":                      N_SEEDS,
    ...
    # Print
    # ---------------------------------------------------------------------------
    onset50_p  = by_onset["50"]["precision"]
    onset200_p = by_onset["200"]["precision"]

    print()
    print(f"V-MV-CONSERVATION v10 (48 cells, all onset times, GAE 0.7.17):")
    print(f"  VarQMonitor (persistence=3, threshold=0.05, baseline=10 overrides):")
    ...
    print()
    print(f"  Series complete (v2-v10):")
    print(f"  v6 no-persist: onset50 P=0.734 ✓ onset200 P=0.301 ✗")
    print(f"  v8 persist bw=50: onset50 P=0.000 ✗ onset200 P=0.603 ✓")
    print(f"  v10 persist bw=10: onset50 P={onset50_p:.3f} "
          f"{'✓' if onset50_p > 0.70 else '✗'} "
          f"onset200 P={onset200_p:.3f} "
          f"{'✓' if onset200_p > 0.70 else '✗'}")
