"""
V-MV-CONSERVATION v5 — self-calibrating h OLSMonitor (GAE 0.7.14).

Identical 48-cell design to v2/v3. Single change: OLSMonitor() with no
args. h is calibrated at plateau via h = σ²_OLS × ln(1000) / (2k),
guaranteeing ARL₀ ≥ 1000 regardless of override rate or window size.

v3 failure: fixed h=5.0 calibrated for σ_OLS≈0.06, gave ARL₀≈217 at
σ_OLS≈0.15 (α=0.25, W=30). 145-decision pre-onset window → 67% FP rate.
v4 fix: at σ_OLS=0.15, h calibrates to ≈0.78 → ARL₀≈1000 → ~13.5% FP.

Save: experiments/v_mv_conservation/results/results_v5.json
"""
...
# ---------------------------------------------------------------------------
# VarQ Monitor (unchanged from v2/v3)
# ---------------------------------------------------------------------------
class VarQMonitor:
...
def run_seed(onset_time: int, deg_profile: str, q_endpoint: float,
    ...
    ols_mon     = OLSMonitor()   # h=None until plateau — self-calibrating
    ...
    for t in range(N_DECISIONS):
        ...
        if is_override:
            ...
            analyst_corr = ai_corr

        ols_val = rolling_ols.record(is_override, analyst_corr)
        if ols_val is not None:
            was_pre = not ols_mon.baseline_frozen
            fired   = ols_mon.update(ols_val)
            if was_pre and ols_mon.baseline_frozen and plateau_t is None:
                plateau_t = t
    ...
    return {
        ...
        "baseline_ols": round(ols_mon.baseline_ols, 4),
...
# Metrics
# ---------------------------------------------------------------------------
def metrics(seed_results: list[dict], out_key: str, lag_key: str) -> dict:
    tp = sum(1 for r in seed_results if r[out_key] == "TP")
    fp = sum(1 for r in seed_results if r[out_key] == "FP")
    fn = sum(1 for r in seed_results if r[out_key] == "FN")
    precision = tp / (tp + fp) if (tp + fp) > 0 else 1.0
    recall    = tp / (tp + fn) if (tp + fn) > 0 else 0.0
    f1        = (2*precision*recall / (precision+recall)
                 if (precision+recall) > 0 else 0.0)
    lags = [r[lag_key] for r in seed_results
            if r[out_key] == "TP" and r[lag_key] is not None]
...
def main():
    ...
    print(f"V-MV-CONSERVATION v5 (GAE {gae.__version__})")
    print(f"  48 cells x {N_SEEDS} seeds = {48*N_SEEDS} runs  "
          f"(OLSMonitor self-calibrating h)", flush=True)
    print()
    ...
    for cell_idx, (onset_time, deg_profile, q_endpoint, (sigma_eff, V)) in enumerate(cells):
        ...
        m_ols  = metrics(cell_sr, "ols_outcome",  "ols_lag")
        m_varq = metrics(cell_sr, "varq_outcome", "varq_lag")

        h_vals = [r["h_calibrated"] for r in cell_sr if r["h_calibrated"] is not None]
        mean_h = float(np.mean(h_vals)) if h_vals else float("nan")
        pt_vals = [r["plateau_t"] for r in cell_sr if r["plateau_t"] is not None]
        mean_pt = float(np.mean(pt_vals)) if pt_vals else float("nan")
        n_no_plateau = sum(1 for r in cell_sr if r["plateau_t"] is None)
        ...
        cell_records.append({
            ...
            "ols":            m_ols,
            "varq":           m_varq,
            "mean_h":         round(mean_h, 4) if not np.isnan(mean_h) else None,
            "mean_plateau_t": round(mean_pt, 1) if not np.isnan(mean_pt) else None,
            "n_no_plateau":   n_no_plateau,
        })
        ...
        print(f"  [{cell_idx+1:02d}/48] onset={onset_time:3d} {deg_profile:6s} "
              ...
              f" h={mean_h:.3f} plt@{mean_pt:.0f} | "
              f"VQ P={m_varq['precision']:.2f} R={m_varq['recall']:.2f}", flush=True)
    ...
    # Global h and plateau stats
    all_h  = [r["h_calibrated"] for r in all_seeds if r["h_calibrated"] is not None]
    all_pt = [r["plateau_t"]    for r in all_seeds if r["plateau_t"]    is not None]
    mean_h_global  = round(float(np.mean(all_h)),  4) if all_h  else float("nan")
    mean_pt_global = round(float(np.mean(all_pt)), 1) if all_pt else float("nan")
    n_no_plateau   = sum(1 for r in all_seeds if r["plateau_t"] is None)

    # By onset_time breakdown
    by_onset: dict[str, dict] = {}
    for ot in ONSET_TIMES:
        sub = [r for r in all_seeds if r["onset_time"] == ot]
        m   = metrics(sub, "ols_outcome", "ols_lag")
    ...
    # ---------------------------------------------------------------------------
    out = {
        "experiment":           "V-MV-CONSERVATION-v5",
        "gae_version":          "0.7.14",
        "n_cells":              48,
        "n_seeds":              N_SEEDS,
        "n_decisions":          N_DECISIONS,
        "mean_h_calibrated":    mean_h_global,
        "mean_plateau_decision": mean_pt_global,
        "n_seeds_no_plateau":   n_no_plateau,
        "ols_monitor": {
            "precision":          ols_mac_p,
            "recall":             ols_mac_r,
        ...
        "best_detector": best,
        "vs_v4": {
            "ols_precision_v4": 0.528,
            "ols_precision_v5": ols_mac_p,
            "improvement_pp":   round((ols_mac_p - 0.528) * 100, 1),
            "onset200_precision_v4": 0.190,
            "onset200_precision_v5": by_onset["200"]["precision"],
        },
        "cells": cell_records,
    }

    out_path = (REPO_ROOT / "experiments" / "v_mv_conservation"
                / "results" / "results_v5.json")
    ...
    # ---------------------------------------------------------------------------
    print()
    print(f"V-MV-CONSERVATION v5 (48 cells, 20 seeds, GAE 0.7.14):")
    print(f"  Mean h calibrated: {mean_h_global:.3f} (σ_OLS-dependent)")
    print(f"  Mean plateau decision: {mean_pt_global:.1f}"
          + (f"  [{n_no_plateau} seeds: plateau never reached]"
             if n_no_plateau > 0 else ""))
    print(f"  OLSMonitor (AC-corrected h):")
    ...
    print(f"  vs v4: OLS precision 0.528 → {ols_mac_p:.3f} "
          f"({(ols_mac_p - 0.528)*100:+.1f}pp)")
    print(f"         onset=200 precision 0.190 → {by_onset['200']['precision']:.3f}")
