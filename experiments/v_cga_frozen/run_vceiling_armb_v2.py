"""
V-CEILING Arm B v2 — GAE 0.7.8 gradient fix rerun
===================================================
Re-runs V-CEILING Arm B (A1×B0 low-discriminating geometry) to confirm
d>0.30 holds under fixed DiagonalKernel gradient. Previous d=0.357 at
GAE 0.7.6 (bugged); gradient bug may have inflated effect.

Condition: A1×B0 structured μ*, threat_intel (dim 2) and pattern_history
(dim 4) flattened to 0.50. Δσ-weighted bootstrap (sigma_before provided).
Gates CLAIM-62 B0 CONDITIONAL-MET status.

Run:
    PYTHONUTF8=1 python experiments/v_cga_frozen/run_vceiling_armb_v2.py
"""
...
FACTOR_NAMES = [
    ...
    "threat_intel_enrichment", # dim 2 — flattened in B0
    "time_anomaly",            # dim 3 — fixed
    "pattern_history",         # dim 4 — flattened in B0
    "device_trust",            # dim 5 — enriched in CD
]
IDX = {f: i for i, f in enumerate(FACTOR_NAMES)}
CD_ENRICHED_DIMS = [0, 5]   # travel_match, device_trust — M4 dims
...
# sigma_before = sigma_after × 1.5 for all factors
SIGMA_BEFORE = {f: round(v * 1.5, 4) for f, v in SIGMA_AFTER.items()}

SV_AFTER  = np.array([SIGMA_AFTER[f]  for f in FACTOR_NAMES])
SV_BEFORE = np.array([SIGMA_BEFORE[f] for f in FACTOR_NAMES])

# W_delta (Δσ scheme) = sigma_before² / sigma_after⁴
# With uniform ×1.5: reduces to 2.25/sigma_after² for all factors
...
# ── Structured A1×B0 μ* ────────────────────────────────────────────────────────
# Same raw geometry as V-CEILING. Dims 2 (threat_intel) and 4 (pattern_history)
# flattened to 0.50 — the "B0" low-discriminating condition.
_MU_STAR_RAW = {
...
def _build_mu_star_b0() -> np.ndarray:
    mu = np.full((N_CATEGORIES, N_ACTIONS, N_FACTORS), 0.5)
    for (cat, act), vec in _MU_STAR_RAW.items():
        mu[CAT_IDX[cat], ACT_IDX[act], :] = vec
    # B0 flattening: threat_intel (dim 2) and pattern_history (dim 4) → 0.50
    mu[:, :, IDX["threat_intel_enrichment"]] = 0.50
    mu[:, :, IDX["pattern_history"]]         = 0.50
    return mu

MU_STAR = _build_mu_star_b0()

# Verify flattening
assert np.all(MU_STAR[:, :, IDX["threat_intel_enrichment"]] == 0.50), "B0 flatten failed dim2"
assert np.all(MU_STAR[:, :, IDX["pattern_history"]]         == 0.50), "B0 flatten failed dim4"
...
def run_one_seed(seed: int) -> dict:
    ...
    learn_rng   = np.random.RandomState(seed + 30000)  # shared — identical sequence
    ...
    for cond, mu0, lr_seed_offset in [("C0", mu0_c0, 0), ("T2", mu0_t2, 0)]:
...
def main():
    print("=" * 65)
    print("V-CEILING Arm B v2 (A1×B0, GAE 0.7.8, gradient fix)")
    print("=" * 65)
    print(f"N_SEEDS={N_SEEDS}, N_BOOTSTRAP={N_BOOTSTRAP}, "
          f"N_POST_BOOTSTRAP={N_POST_BOOTSTRAP}")
    print(f"Condition: A1×B0 (threat_intel+pattern_history flattened to 0.50)")
    print(f"sigma_before = sigma_after × 1.5 for all factors")
    print()
    print("W_delta values (sigma_before = 1.5×sigma_after → uniform 2.25/sa²):")
    ...
    v1_d   = 0.357
    v2_d   = abs(m2["cohens_d"])
    verdict = "PASS" if m2["pass"] else "FAIL"

    if v2_d > 0.30:
        comparison_verdict = "CONFIRMED"
    elif v2_d > 0.20:
        comparison_verdict = "REDUCED"
    else:
        comparison_verdict = "NO LONGER PASSES"

    if m2["pass"]:
        claim_status = ("CLAIM-62 B0 CONDITIONAL-MET confirmed at GAE 0.7.8. "
                        "d>0.30 holds with fixed gradient.")
    elif v2_d > 0.20 and float(m2["p_value"]) < 0.01:
        claim_status = (f"CLAIM-62 B0 REDUCED. d={v2_d:.3f} (was {v1_d:.3f}). "
                        "p<0.01 but d<0.30 gate. Gradient bug partially inflated v1 result.")
    else:
        claim_status = (f"CLAIM-62 B0 NO LONGER PASSES at GAE 0.7.8. "
                        f"d={v2_d:.3f}, p={m2['p_value']:.4f}.")

    # ── Save ─────────────────────────────────────────────────────────────────────
    results = {
        "experiment":   "V-CEILING-ARM-B-v2",
        "version":      "v2_gradient_fix_gae_0.7.8",
        "gae_version":  "0.7.8",
        "date":         "2026-03-24",
        "n_seeds":      N_SEEDS,
        "condition":    "A1xB0 structured mu*, low-discriminating enriched factors",
        "sigma_after":  SIGMA_AFTER,
        "sigma_before": SIGMA_BEFORE,
        "w_delta_values": W_DELTA_VALUES,
        "previous_result": {
            "d": v1_d, "p": "<0.0001", "m2_pass": True,
            "cause_of_rerun": "DiagonalKernel gradient bug",
        },
        "m2": m2,
        "m4_enriched": {
            "c0": round(m4["c0_dist"], 4),
            "t2": round(m4["t2_dist"], 4),
            "reduction_pct": m4["reduction_pct"],
            "pass": m4["pass"],
        },
        "m4_total": m4t,
        "day1_accuracy": d1,
        ...
        "claim62_b0_status": claim_status,
        "runtime_s": round(elapsed_total, 1),
    }

    out_dir  = Path(__file__).parent / "results"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "results_vceiling_armb_v2.json"
    ...
    print()
    print("=" * 65)
    print(f"V-CEILING Arm B v2 (GAE 0.7.8, gradient fix): {verdict}")
    print("=" * 65)
    print(f"M2: C0={m2['n_half_c0']:.1f}, T2={m2['n_half_t2']:.1f}, "
          f"reduction={m2['reduction_pct']:.1f}%, "
          f"p={m2['p_value']:.4f}, d={v2_d:.3f} [{_pf(m2['pass'])}]")
    print(f"  diff CI95: [{m2['diff_ci95'][0]:.2f}, {m2['diff_ci95'][1]:.2f}] decisions")
    print(f"M4_enriched (dims 0,5): C0={m4['c0_dist']:.3f}, T2={m4['t2_dist']:.3f} "
          f"[{_pf(m4['pass'])}]")
    ...
    print()
    print(f"vs v1 (bugged): d={v1_d:.3f} -> v2 (fixed): d={v2_d:.3f}  [{comparison_verdict}]")
    print(f"Power at d={powa['observed_d']:.3f}: {powa['power_at_n200']:.1%} (N=200)")
    print()
    if m2["pass"]:
        print("CLAIM-62 B0 CONDITIONAL-MET confirmed at GAE 0.7.8.")
    elif v2_d > 0.20 and float(m2["p_value"]) < 0.01:
        print(f"CLAIM-62 B0 gate no longer met. New d={v2_d:.3f}. "
              f"Direction correct (p={m2['p_value']:.4f}).")
    else:
        print(f"CLAIM-62 B0 gate no longer met. d={v2_d:.3f}, p={m2['p_value']:.4f}.")
