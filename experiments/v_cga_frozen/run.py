"""
V-CGA-FROZEN: Graph Compounding with Frozen Centroids
======================================================
Tests whether graph enrichment (lower per-factor sigma) during a centroid-
frozen period improves post-unfreeze convergence.

Mechanism: FactorComputers produce richer (lower-noise) factors as the graph
accumulates more ThreatIndicators, entity-resolution matches, and SIEM history.
During the frozen period these factors train the alert pipeline — when centroids
unfreeze they converge faster because the factor space is tighter.

Three metrics:
  M1 — sigma_per_factor reduction: enriched factors show > 10% sigma drop in A vs B
  M2 — N_half: post-unfreeze convergence speed (decisions to reach final_acc - 2pp)
  M3 — IKS trajectory: normalized centroid-recovery score at decision 300

PASS: all three metrics pass.  FAIL: any one metric fails.

Run:
    PYTHONUTF8=1 python experiments/v_cga_frozen/run.py
"""
...
def run_one_seed(seed: int) -> dict:
    ...
    Returns dict with per-condition sigma observations, accuracy trajectories,
    and IKS checkpoints.
    """
...
def main():
    print("=" * 65)
    print("V-CGA-FROZEN: Graph Compounding with Frozen Centroids")
    print("=" * 65)
    ...
    print("\n" + "=" * 65)
    print(f"V-CGA-FROZEN: {verdict}")
    print("=" * 65)
    print(f"Metric 1 (sigma reduction):  {'PASS' if m1['pass'] else 'FAIL'}")
    ...
    print()
    print(f"Metric 2 (N_half):           {'PASS' if m2['pass'] else 'FAIL'}")
    ...
    print()
    print(f"Metric 3 (IKS trajectory):   {'PASS' if m3['pass'] else 'FAIL'}")
    ...
    if verdict == "PASS":
        ...
        print("DESIGN GAP — GraphAttentionBridge required")
    print(design_implications)
    print("=" * 65)
