"""
V-CGA-FROZEN v2 — KernelWeightRefresh fix (GAE 0.7.1)
======================================================
Re-run of V-CGA-FROZEN with ProfileScorer.kernel_weight_refresh() called after
each 10-day enrichment step in condition A.

Root cause of v1 M2 FAIL: DiagonalKernel weights stuck at Day 0 sigma values
during frozen window.  Fix: kernel_weight_refresh(covariance_estimator) refreshes
DiagonalKernel weights from accumulated CovarianceEstimator data; safe during freeze
(learning_enabled=False, mu never touched).

Change vs v1:
  - CovarianceEstimator tracks factor observations in BOTH conditions.
  - Condition A only: scorer.kernel_weight_refresh(cov_est) called after each
    10-day batch (days 10, 20, …, 90).
  - All parameters, sigma schedules, and pass criteria are IDENTICAL to v1.

Run:
    PYTHONUTF8=1 python experiments/v_cga_frozen/run_v2.py
"""
...
# ── Canonical parameters (identical to v1 — do not change) ────────────────────
N_SEEDS         = 50
N_BOOTSTRAP     = 1200
N_FROZEN_DAYS   = 90
...
def main():
    print("=" * 65)
    print("V-CGA-FROZEN v2 (with KernelWeightRefresh — GAE 0.7.1)")
    print("=" * 65)
    ...
    results = {
        ...
        "version":      "v2_with_kernel_weight_refresh",
        ...
        "comparison_to_v1": {
            "m2_p_value_v1":    0.097324,
            "m2_p_value_v2":    m2["p_value"],
            "m2_effect_size_v1": 0.2415,
            "m2_effect_size_v2": m2["effect_size_d"],
            "n_half_a_v1":      75.1,
            "n_half_a_v2":      m2["n_half_A_mean"],
            "n_half_b_v1":      93.3,
            "n_half_b_v2":      m2["n_half_B_mean"],
        },
    }
    ...
    print("\n" + "=" * 65)
    print(f"V-CGA-FROZEN v2 (with KernelWeightRefresh): {verdict}")
    print("=" * 65)
    ...
    print(f"  v1 → v2: N_half_A {results['comparison_to_v1']['n_half_a_v1']} → {m2['n_half_A_mean']:.1f}  "
          f"N_half_B {results['comparison_to_v1']['n_half_b_v1']} → {m2['n_half_B_mean']:.1f}")
    print(f"  v1 → v2: p {results['comparison_to_v1']['m2_p_value_v1']:.6f} → {m2['p_value']:.6f}  "
          f"d {results['comparison_to_v1']['m2_effect_size_v1']:.4f} → {m2['effect_size_d']:.4f}")
    print()
