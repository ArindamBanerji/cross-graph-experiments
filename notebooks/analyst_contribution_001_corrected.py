# ANALYST-CONTRIBUTION-001 — DESIGN CORRECTION RUN (one-time only)
# Original Cond A failed: q_high=0.90, q_low=0.55, mean q̄=0.725 < 0.85 baseline.
# Level 2 correctly fired on mean quality drop (87% seeds). Design gave it real signal.
# Mechanism confirmed: Var=0.0934 >> threshold=0.01, Level 3 fired 100%.
# Corrected: q_high=0.95, q_low=0.75, mean q̄=0.85 (healthy baseline).
# If FAIL at corrected parameters: CLAIM-ACM-01 = production milestone. HARD STOP.
"""
ANALYST-CONTRIBUTION-001 CORRECTED — per-analyst OLS variance as Level 3 detector.

MAP v4.3.2 design correction. N_DECISIONS=1200, N_WARMUP=400, N_ANALYSTS=10, N_SEEDS=30.
VAR_OLS_THRESHOLD=0.01.

Design correction from original (GATE FAIL):
  Original: q_high=0.90, q_low=0.55 → mean q̄=0.725 < baseline=0.85.
  Level 2 fired on mean quality degradation (87% seeds). Not a design flaw in Level 3;
  Level 3 correctly identified bimodal structure (Var=0.0934 >> 0.01, 100% detection).
  Gate ≤30% for Level 2 requires aggregate OLS near 1.0 (healthy mean).
  Corrected: q_high=0.95, q_low=0.75 → mean q̄=0.85 = healthy baseline.
  Expected Var(OLS_i) ≈ 0.030 (spread 0.20 vs original 0.35).

Save: notebooks/analyst_contribution_001_results.json (overwrites original run)
"""
...
# CORRECTED quality values (original: Q_HIGH=0.90, Q_LOW=0.55)
Q_HIGH    = 0.95
Q_LOW     = 0.75
Q_UNIFORM = 0.65
...
def main():
    ...
    print(f"ANALYST-CONTRIBUTION-001 CORRECTED (q={Q_HIGH}/{Q_LOW}, "
          f"N={N_SEEDS}, GAE {gae.__version__}):")
    print()

    # Pre-calculations
    meas_window    = N_DECISIONS - N_WARMUP
    ov_per_analyst = meas_window * ALPHA / N_ANALYSTS
    mean_q_bimodal = (Q_HIGH + Q_LOW) / 2.0
    expected_var   = float(np.var([Q_HIGH / 0.85] * 5 + [Q_LOW / 0.85] * 5))
    ...
    print(f"  Bimodal mean q: {mean_q_bimodal:.2f} (target = baseline 0.85)")
    print(f"  OLS spread: {Q_HIGH/0.85:.3f} vs {Q_LOW/0.85:.3f} "
          f"(spread={Q_HIGH/0.85 - Q_LOW/0.85:.3f})")
    print(f"  Expected Var(OLS_i) bimodal: {expected_var:.4f} >> threshold={VAR_OLS_THRESHOLD}")
    print()
    ...
    # ---- Condition A ----
    print("  Running Condition A (bimodal corrected)...", flush=True)
    ...
    # ---- Save ----
    out = {
        "experiment":  "ANALYST-CONTRIBUTION-001-CORRECTED",
        "gae_version": gae.__version__,
        "design_note": (
            "Corrected q_high=0.95, q_low=0.75. "
            "Original q_high=0.90, q_low=0.55 was design error: "
            "mean q=0.725 < 0.85 baseline triggered Level 2 on mean quality drop."
        ),
    ...
    print(f"ANALYST-CONTRIBUTION-001 CORRECTED (q={Q_HIGH}/{Q_LOW}, "
          f"N={N_SEEDS}, GAE {gae.__version__}):")
    print()
    print(f"Condition A — Bimodal corrected (5xq={Q_HIGH}, 5xq={Q_LOW}, mean={mean_q_bimodal:.2f}):")
    print(f"  Mean aggregate OLS:         {mean_agg_ols_a:.3f} [sanity: 0.90-1.15]")
    print(f"  Mean Var(OLS_i):            {mean_var_ols_a:.4f} [sanity: >0.010]")
    print(f"  Mean analysts >=5 overrides: {mean_n_qual_a:.1f}{insufficient}")
